...
function refreshCart() {
                ...
                <div class="cart_item__text-box">
                    ...
                    <div>
                        ...
                        <span class="cart-item__price-overall">$${renderPrice(
                          matchingItem.price * item.quantity
                        )}</span>
                    </div>
                </div>
  ...
  document.querySelector(".js-cart").innerHTML =
    `<h1 class="cart-quantity">Your Cart (${cartTotalQuantity})</h1>` +
    cartHTML +
    `
...
export function refreshFullCart() {
  if (cart.length > 0) {
    ...
    // confirm
    document
      .querySelector(".cart-confirm-order")
...
function addToModal() {
  ...
  cart.forEach((item) => {
    ...
    itemsPreviewHTML += `
      ...
      matchingItem.category
            ...
            <span class="modal__total-price">$${renderPrice(
              matchingItem.price * item.quantity
            )}</span>
        </div>
        `;
  ...
  document.querySelector(".items-preview").innerHTML =
    itemsPreviewHTML +
    `
